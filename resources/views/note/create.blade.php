...
<x-app-layout>
    ...
    <div class="container mx-auto bg-green-600 pb-8">
        ...
        <div class="w-3/4 mx-auto py-4 text-start">
            <h1 class="text-3xl text-white px-6 pt-4 pb-2">New Note</h1>
        </div>

        <form class="w-3/4 py-4 mx-auto bg-white shadow-lg rounded-md pb-2 mb-2" action="{{ route('note.store') }}" method="POST">
            ...
            <div class="flex flex-wrap mx-4 mb-6">
                <div class="w-full px-3">
                    ...
                    <input class="appearance-none block w-full placeholder-gray-400 bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800" name="title" id="title" type="text" placeholder="Title" value="{{ old('title') }}"></input>
                    @error('title')
            ...
            <div class="flex flex-col md:flex-row mx-7 mb-6 gap-6 justify-between items-stretch">
                <div>
                    ...
                    <div class="md:text-center py-2">
                        <input class="appearance-none rounded-sm border-2 border-green-600 text-green-600 focus:ring-green-600 outline-none focus:ring-0 checked:bg-green-500" name="pending" id="pending" type="checkbox" value="true"
                               @if (old('pending') == 'true') checked @endif></input>
                    </div>
                    @error('pending')
                        <p class="text-red-600 text-md italic">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input class="appearance-none block w-full placeholder-gray-400 bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800" name="date" id="date" type="date" min="2024-01-01" value="{{ old('date') }}"></input>
                    @error('date')
                        <p class="text-red-600 text-md italic">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input class="appearance-none block w-full placeholder-gray-400 bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800" name="date_limit" id="date_limit" type="date" min="2024-01-01" value="{{ old('date_limit') }}"></input>
                    @error('date_limit')
                        <p class="text-red-600 text-md italic">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-wrap mx-7 mb-6">
                ...
                <ul class="flex flex-wrap justify-start">
                    @foreach ($tags as $x => $tag)
                        <li class="inline-flex items-center gap-x-2 py-2 px-1 text-sm font-medium rounded-md bg-green-200 border-2 border-green-400 m-1">
                            <div class="relative flex items-start w-full">
                                <div class="flex items-center h-5 ">
                                    ...
                                    <input class="appearance-none rounded-sm text-green-600 outline-none focus:ring-0 checked:bg-green-500" type="checkbox" id="<?php echo $tag->name; ?>" name="tag[]" value="<?php echo $tag->id; ?>"
                                           @if (old('tag')) @for ($i = 0; $i <= count(old('tag')); $i++) 
    ...
    <div class="flex flex-wrap mx-4 mb-6">
        <div class="w-full px-3">
            ...
            <input class="appearance-none block w-full placeholder-gray-400 bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800" name="rating" id="rating" step="any" type="number" placeholder="Rating" value="{{ old('rating') }}"></input>
            @error('rating')
                <p class="text-red-600 text-md italic">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="flex flex-wrap mx-4 mb-6">
        <div class="w-full px-3">
            ...
            <input class="appearance-none block w-full placeholder-gray-400 bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800" name="url" id="url" type="text" placeholder="http://" value="{{ old('url') }}"></input>
            @error('url')
                <p class="text-red-600 text-md italic">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="flex flex-wrap mx-4 mb-6">
        <div class="w-full px-3">
            ...
            <textarea rows="8" cols="50" name="info" id="info" type="text" class="appearance-none block w-full bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800">{{ old('info') }}</textarea>
            @error('info')
                <p class="text-red-600 text-md italic">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div class="flex flex-wrap mx-4 mb-6">
        <div class="w-full px-3">
            ...
            <textarea rows="8" cols="50" name="comment" id="comment" type="text" class="appearance-none block w-full bg-gray-100 text-black border-2 border-green-600 rounded py-3 px-4 mb-3 leading-tight focus:ring-0 focus:border-green-800">{{ old('comment') }}</textarea>
            @error('comment')
                <p class="text-red-600 text-md italic">{{ $message }}</p>
            @enderror
        </div>
